...
Timer::Timer(uint64_t timeout, function<bool()> cb, const shared_ptr<TcpConnection> client)
: _cb(cb)
, _client(client)
, _deleted(false)
{
...
void Timer::setTimerCb(function<bool()> &cb)
{
    _cb = cb;
}
...
void TimerManager::tick()
{
    ...
    while(!_is_exit)
    {
        ...
        while(_time_heap.size())
        {
            ...
            {
                //_time_heap.top()->_client->shutdown();//关闭
                _time_heap.pop();
            }
            //还没到时间
...
TimerManager& TimerManager::getInstance(int waitTime)
{
    mutex _mutex;
