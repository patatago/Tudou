...
int TcpConnection::send(const char *msg, size_t len)
{
    if(_channel)
    {
        ...
        if(ret.second == -2 || ret.second == -1) //sigppe
        {
            Debug << "recv sigpipe or client is closed" << std::endl;
            _channel->close();
            return -1; //发送不成功
        }
...
{	
    ...
    {
        Warn << "client handshaking error" << std::endl;
        return ;
    }
...
int TcpServer::listen(const string &host, uint16_t port)
{
    ...
    Info << "this is a new listen sockfd" << std::endl;
