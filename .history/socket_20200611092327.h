	...
	sendPart(int sockfd, const char *msg, size_t len)
	{
		...
		while(1)
		{
			...
			if(ret <= 0)
			{
				...
				{	
					if(errno == EPIPE) {
						std::cout << "1111111111111188888811111111111111" << std::endl;
		...
		while(1)
		{
			...
			if(ret <= 0)
			{
				...
				else if(errno == EAGAIN)
				{
					...
					else 	{ return make_pair(_error, 0); }  //部分发送
					
				}
				else if(errno == EPIPE) 
				{ 
					std::cout << "111111111111nimade1111111111111111" << std::endl;
					_error.setErrno(errno);
					return make_pair(_error, -2);
				}
				else
				{
					//if(errno == SIGPIPE) std::cout << "1111111111111111111111111111" << std::endl;
					_error.setType(Error::ERROR);
					return make_pair(_error, -1);
				}
