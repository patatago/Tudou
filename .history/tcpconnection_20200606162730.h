...
private:
	...
	void handleRead(const Ptr &ptr)
	{
		...
		if(ptr->_state == TcpState::HANDSHAKING && !handleHandshake(ptr)) //判断是否链接
		{
			return ;
		}
		//std::cout << "this is handle" << std::endl;
		
		if(ptr->_state == TcpState::CONNECTED && _channel->getFd() > 0)
		{
	...
	{
		if(ptr->_state == TcpState::HANDSHAKING)
		{
			handleHandshake(ptr);
		}
		else if(ptr->_state == TcpState::CONNECTED)
		{
			//没有数据可以发送
