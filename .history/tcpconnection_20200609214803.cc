...
, _output()
{
    ...
    _name = static_cast<uint64_t>(this);
...
{	
    std::cout << " this is a reda" << std::endl;
...
void TcpServer::handleAccept(shared_ptr<TcpServer> _conn) //listen readcb
{
    ...
    _conn->_connections.insert(con); //有一个链接加入
                    ...
                    con->setCloseCb([&, this](const TcpConnection::Ptr &s) 
                                    {
                                        ...
                                        // if(_conn->_connections.find(s) != _conn->_connections.end())
                                        // {
                                        //     std::cout << " this is=on: " << std::endl;
                                        //     _conn->_connections.erase(s);
                                        // }
