...
public:
	...
	{
		string _message_line = buffer.read(len - bufer->getReadPos() + 2); //读出请求行
	...
	{
		string _message_line = buffer.read(len - bufer->getReadPos() + 2); //读出请求头
	...
	bool parse(Buffer &buffer)
	{
		...
		while(true)
		{
			...
			else if(_state == PARSE_BODY)
			{
				...
				//std::cout << "_message: " << _message << std::endl; 
