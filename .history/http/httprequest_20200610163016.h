...
public:
	...
	bool parseRequestLine(int len, Buffer &buffer)
	{
		...
		std::cout << "*******************" << std::endl;
		std::cout << _message_line << std::endl;
		std::cout << "*******************" << std::endl;
		...
		std::cout << "*******************"<< _message_line[pos2+8] << std::endl;
...
public:
	...
	void clientError(const string &cause, const string &errnum, const string &shortmsg, const string &longmsg, const TcpConnection::Ptr &ptr)
	{
		stringstream ss, aa;
		...
		ptr->send(aa.str() + ss.str());
		//_buffer.append(aa.str());
		//_buffer.append(ss.str());
		//std::cout << aa.str() << std::endl;
	}
	
	void append(const TcpConnection::Ptr &ptr)
	{
		
		if(_message._method != (HttpMessage::POST || HttpMessage::GET))
		{
			clientError(_message.getFilename(), "501", "Not Implemented", "Tudou cannot implement this method", ptr);
			return ;
		}
		if(stat(_message.getFilename().c_str(), &sbuf) < 0)
		{
			clientError(_message.getFilename(), "404", "Not Found", "Tudou cannot find this file", ptr);
			return ;
		}
		if(!(S_ISREG(sbuf.st_mode)) ||!(sbuf.st_mode & S_IRUSR))
        {
            clientError(_message.getFilename(), "403", "Not Found", "Tiny couldn't read this file", ptr);  
            return ;    
        }
		getFileType();
		stringstream ss;
    	...
    	close(srcfd);
		ptr->send(ss.str() + string(srcp, sbuf.st_size) + "\r\n\r\n");
		//_buffer.append(ss.str());
