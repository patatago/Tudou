...
public:
	...
	static int setNonBlock(int sockfd, bool flag)
	{
		...
		{
			old_opt |= O_NONBLOCK;
					std::cout << std::endl<<strerror(errno) << "0------1--" << "+++++"<<std::endl;
			auto ret = ::fcntl(sockfd, F_SETFL, old_opt);
			std::cout << std::endl<<strerror(errno) << "0------1--" << "+++++"<<std::endl;
			return ret;
		}
		else
	...
	readPart(int sockfd, Buffer &_buffer)
	{
		...
		while(1)
		{
			...
			ret = ::read(sockfd, _temp_buffer, sizeof(_temp_buffer) - 1);
