...
private:
	...
	{	
		...
		if(_state == CONNECTED && _channel->getFd() > 0)
		{
			...
			std::cout << "zuil888" << std::endl;
			if(ret.second == -1)
			{
				std::cout << "zuilqwe9" << std::endl;
				return ;
			}
			std::cout << "zuile9" << std::endl;
	...
	void handleWrite(const Ptr &ptr)
	{
		...
		if(_state == TcpConnection::HANDSHAKING)
		{
			handleHandshake(ptr);
		}
		std::cout << "bye2" << std::endl;
