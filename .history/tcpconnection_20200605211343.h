...
public:
	...
	*/
	...
	EventLoop::Ptr getEventLoop() { return _loop; }
...
private:
	...
	void handleRead(const Ptr &ptr)
	{
		...
		while(_state == CONNECTED && _channel->getFd() > 0)
		{

			//开始读数据到缓冲区，-1失败-2断开连接	,读了ret.second回来
			//std::cout << "readpart --" << std::endl;
			auto ret = SocketTool::readPart(_channel->getFd(), _buffer, _MAX_READ_SIZE_);	//每次最多读_MAX_READ_SIZE_	
			...
			//std::cout << ret.second << "----" << strlen(_buffer) << std::endl;
