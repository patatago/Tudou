...
public:
	...
	sendPart(int sockfd, Buffer &_buffer)
	{
		...
		while(1)
		{
			...
			//string _message_line = _buffer.read(pos - _buffer.getReadPos() + 2); 
			//std::cout << "_message_line: " <<_message_line<< std::endl;
			ret = ::write(sockfd, _buffer.begin(), len);
			if(ret <= 0)
			{
				if(errno == EINTR)
				{
	...
	readPart(int sockfd, Buffer &_buffer)
	{
		...
		while(1)
		{
			ret = ::read(sockfd, _temp_buffer, 1024);
			...
			// recvPeek(sockfd, _temp_buffer, 1024); //看一下sockfd有没有数据	
			// if(ret == -2) //读完了，errno=eagain
			// {
			// 	//_error.setType(Error::ERROR);
			// 	//_error.setErrno(errno);
			// 	return make_pair(_error, 0);
