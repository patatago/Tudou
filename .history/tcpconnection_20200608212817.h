...
public:
	...
	//connect封装
	static Ptr connection(EventLoop::Ptr loop, const string &host, uint16_t port, int timeout = 10, const char *localIp = "0.0.0.0");
	//设置回调函数
...
{
public:
	static void start(EventLoop::Ptr loop, const string &host, uint16_t port, int timeout = 10)
	{
		TcpConnection::Ptr client(new TcpConnection(loop, 0));
		_client = client->connection(loop, host, port, timeout, nullptr);
		return _client;
	}
private:
	TcpConnection::Ptr &_client;
};
